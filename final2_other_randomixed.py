...
from idea_util import get_pred_mine, get_pred_random, get_pred_sir_original, get_pred_cobra_original, get_pred_gradient_original
import csv
...
for name in data_names:
    ...
    for fold_idx in range(10):
        ...
        #save all variables in npy files
        ...
        ## Original
        y_sir, y_sir_kernel , time_taken_in_search, time_taken_in_prediction = get_pred_random(dist_train_cobra_original, dist_test_cobra_original, y_train_l.flatten())
        result_df = result_df.append({'dataset': name, 'fold': fold_idx, 'model': 'mine', 'mse': mean_squared_error(y_test, y_sir), 'R2': r2_score(y_test, y_sir) , 'data_trained_on': 'original', 'data_tested_on': 'original', 'time_taken_fit': time_taken_in_search, 'time_taken_infer': time_taken_in_prediction}, ignore_index=True)
        
        ## Split
        y_sir_split, y_sir_kernel_split , time_taken_in_search_split, time_taken_in_prediction_split = get_pred_random(dist_train_cobra_split, dist_test_cobra_split, y_train.flatten())
        result_df = result_df.append({'dataset': name, 'fold': fold_idx, 'model': 'mine', 'mse': mean_squared_error(y_test, y_sir_split), 'R2': r2_score(y_test, y_sir_split) , 'data_trained_on': 'split', 'data_tested_on': 'split', 'time_taken_fit': time_taken_in_search_split, 'time_taken_infer': time_taken_in_prediction_split}, ignore_index=True)
        
        ## No Split
        y_sir_no_split, y_sir_kernel_no_split , time_taken_in_search_no_split, time_taken_in_prediction_no_split = get_pred_random(dist_train_cobra_no_split, dist_test_cobra_no_split, y_train.flatten())
        result_df = result_df.append({'dataset': name, 'fold': fold_idx, 'model': 'mine', 'mse': mean_squared_error(y_test, y_sir_no_split), 'R2': r2_score(y_test, y_sir_no_split) , 'data_trained_on': 'no_split', 'data_tested_on': 'no_split', 'time_taken_fit': time_taken_in_search_no_split, 'time_taken_infer': time_taken_in_prediction_no_split}, ignore_index=True)

        


result_df.to_csv("cobra_result_randomized.csv")
