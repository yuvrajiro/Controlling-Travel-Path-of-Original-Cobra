...
for name in data_names:
    ...
    for fold_idx in range(10):
        ...
        # assert dist_test_cobra_original.shape == (len(x_test_transformed_dk), len(dl_transformed), len(models)) , f"Shape is {dist_test_cobra_original.shape}"
        ...
        # Check if the loaded files are same as the calculated ones

        # dist_train_cobra_original_loaded = np.load(f"dist_train_cobra_original_{name}_{fold_idx}.npy")
        # dist_test_cobra_original_loaded = np.load(f"dist_test_cobra_original_{name}_{fold_idx}.npy")
        # dist_train_cobra_split_loaded = np.load(f"dist_train_cobra_split_{name}_{fold_idx}.npy")
        # dist_test_cobra_split_loaded = np.load(f"dist_test_cobra_split_{name}_{fold_idx}.npy")
        # dist_train_cobra_no_split_loaded = np.load(f"dist_train_cobra_no_split_{name}_{fold_idx}.npy")
        # dist_test_cobra_no_split_loaded = np.load(f"dist_test_cobra_no_split_{name}_{fold_idx}.npy")

        # print(f"Original COBRA : {np.allclose(dist_train_cobra_original, dist_train_cobra_original_loaded)}")
        # print(f"Split COBRA : {np.allclose(dist_train_cobra_split, dist_train_cobra_split_loaded)}")
        # print(f"No Split COBRA : {np.allclose(dist_train_cobra_no_split, dist_train_cobra_no_split_loaded)}")
        # print(f"Original COBRA : {np.allclose(dist_test_cobra_original, dist_test_cobra_original_loaded)}")
        # print(f"Split COBRA : {np.allclose(dist_test_cobra_split, dist_test_cobra_split_loaded)}")
        # print(f"No Split COBRA : {np.allclose(dist_test_cobra_no_split, dist_test_cobra_no_split_loaded)}")


 
result_df.to_csv('results_baseline.csv')
